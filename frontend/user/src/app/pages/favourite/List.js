...
function parseRowData(rowData) {
    ...
    if( Array.isArray(rowData) && rowData.length > 0){
      rowData.forEach(function(item, index){
        JsonArray.push(createDataArray( item.id, (item.favouritable && item.favouritable.id) ? item.favouritable.name : ' --- ' ,false));
      });
    }
    return JsonArray;
}
...
export default function EnhancedTable(props) {
  ...
  function getFavourite(search) {
    let filter = {};
    if(search != null)
      filter = {...filter, ...search }

     list('favourites', filter).then(function (response) {
  ...
  function handleSearch(event) {
    getFavourite({search:event.target.value});
  }
  ...
  function confirm(event) {
      del('favourites/'+rows[userIndex].id+'/').then(function (response) {
