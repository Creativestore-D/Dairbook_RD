  ...
  function loadDatatable() {
    ...
    let columnDefs = [
      ...
      },
      {
        ...
        "createdCell": function (td, cellData, rowData, row, col) {
          ReactDOM.render(
          <a onClick={() => history.push("/"+USER_URL+"/aircraft/asset/"+rowData.id) }>
          {cellData}
          </a>
              , td
          );
  ...
  function confirm(event) {
    if(action === 'status' || action === 'is_featured') {
      ...
      patch('aircrafts/'+rows[userIndex].id+'/', data).then(function (response) {
        ...
        if(action === 'is_featured') 
          statusType = response.data.is_featured === 1 ? 'activated' : 'deactivated';
    ...
    <div className={classes.root}>
      ...
      <Paper className={classes.paper}>
        ...
        <div className="kt-portlet kt-portlet--mobile">
            <EnhancedTableToolbar setTemp={setTemp} setRowsData={setRowsData} rows={rows} setSelectedMany={setSelectedMany} setSelected={setSelected} setPlanModalDisplay={setPlanModalDisplay} selected={selected} numSelected={selected.length} type={type} />
