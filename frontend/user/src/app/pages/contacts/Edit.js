...
import {list, patch, RELIGIONS, GENDERS, CONTACT_METHODS, TITLES, MEDIA_URL} from "../../crud/api";
  ...
  getContact(contact_id) {
    list('contacts/'+contact_id+'/').then(
      (response) => {
          ...
          if(response.data.country.id != undefined && response.data.country.id != "") {
            this.setDynamicDropdowns(response.data.country.id, 'country');
          }
          if(response.data.state.id != undefined && response.data.state.id != "") {
